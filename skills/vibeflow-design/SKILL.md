...
description: "以审批通过的 brief 为输入产出架构/设计文档，回答 HOW。内置问题探索（absorbed from brainstorming）— 当问题不清晰时在设计前先厘清。"
---
...
0. **[问题探索]** — 如果无 brief 或问题不清晰，先厘清问题和方向
1. **读取 brief 和 UCD（如需）** — 从 `docs/changes/<change-id>/`
2. **探索技术上下文** — 现有代码、框架、部署环境
3. **读取项目规则** — 从 `rules/` 提取本次设计适用的约束并写入设计文档
4. **提出 2-3 个方案** — 带权衡和推荐
5. **逐节设计审批** — 架构、数据模型、API、UI/UX、测试、部署（用户参与式审批）
...
- **如果存在**：读取其内容作为当前 change 的问题定义和边界输入
- **如果不存在**：继续以下探索步骤
...
## 步骤 1：读取 Brief 和 UCD（如需）并提取设计输入

### 1.0 判断是否需要 UCD

1. 运行 `python scripts/get-vibeflow-paths.py --json`
2. 读取 `docs/changes/<change-id>/brief.md`
2. 检查 SRS 中是否包含 UI 相关功能需求（FR 含用户界面、页面或组件）
3. 检查 `.vibeflow/workflow.yaml` 中的 `template`：
...
- 先读取 `skills/vibeflow-design/references/frontend-design-constraints.md`
- 这些约束**仅在本次设计涉及前端/UI 时生效**
- 在进入视觉风格方案前，先完成下方 4 个前置问题

#### 1.2.0 锁定前端设计方向

在写任何 UI/UX 方案前，先向用户明确这 4 个问题：

1. **谁在用，使用场景是什么**
2. **审美方向是什么**
3. **用户最后会记住什么**
4. **硬约束是什么**（框架、无障碍、性能、终端设备、品牌限制等）

规则：

- 一次只推进必要问题，不要堆一串开放式追问
- 审美方向必须具体命名，禁止使用 `clean and modern` 这类空话
- 在进入后续子步骤前，用一句话复述最终选定的视觉方向
- 必须明确本项目采用的**单一样式策略**：`Tailwind only` / `CSS Modules only` / `CSS-in-JS only`
...
- 必须说明整体空间策略：高密度 / 宽松留白
- 如果桌面审美与移动端约束冲突，要在设计文档中写出 tradeoff

**图标风格：** 线框/填充/双色、图标库（如 Lucide Icons）。

**动效预算：**
- 说明关键动效放在哪些状态
- entrance 200-400ms，exit 150-250ms
- 只保留影响感知和反馈的动效，不要把装饰性微动效写满全局
...
如果项目根目录存在 `rules/`，这是设计阶段的**一等输入**，不是实现阶段才补读的附件。
...
3. 将输出原样写入 `docs/changes/<change-id>/design.md` 的固定章节：

```markdown
## Project Rules And Constraints
```

4. 后续架构、模块边界、接口兼容性、测试策略都必须显式遵守该章节。
5. 如果某条规则需要例外，必须在设计文档中写清：
   - 例外原因
   - 风险
   - 回收计划
...
5. **UI/UX 方案**（如适用）— 布局策略、交互模式
   ...
   - 必须通过 `frontend-design-constraints.md` 自查常见模板味陷阱，避免默认 hero / 默认卡片网格 / 默认导航栏
...
**执行交接硬要求：**
- 当 `rules/` 存在且有适用规则时，设计文档必须包含 `## Project Rules And Constraints`
...
设计阶段完成后，必须向用户展示：
- `design.md` 的关键方案摘要
- Build Contract 与 Implementation Contract 的覆盖范围
